...
else
{
	...
	<table class="table table-bordered table-striped">
		<thead>
			<tr>
				...
				<!--<th class="model"><?php echo JText::_('ESHOP_MODEL'); ?></th>-->
		...
		<tbody>
			...
			foreach ($this->cartData as $key => $product)
			{
				...
				<tr>
					...
					<!--<td class="model"><?php echo $product['product_sku']; ?></td>-->
					<td>
						<input type="text" class="input-mini" value="<?php echo $product['quantity']; ?>" name="quantity[<?php echo $key; ?>]" id="quantity_<?php echo $key; ?>">
						<input type="image" alt="<?php echo JText::_('ESHOP_UPDATE'); ?>" title="<?php echo JText::_('ESHOP_UPDATE'); ?>" src="<?php echo JUri::base(true); ?>/components/com_eshop/assets/images/update.png" onclick="updateCart('<?php echo $key; ?>');" />
				...
				<tr>
					<td class="form-horizontal">
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="coupon_code" name="coupond_code" class="input-large" value="<?php echo $this->coupon_code; ?>">
				...
				<tr>
					<td class="form-horizontal">
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="voucher_code" name="voucher_code" class="input-large" value="<?php echo $this->voucher_code; ?>">
								<button type="button" class="btn btn-small btn-primary" onclick="applyVoucher();"><?php echo JText::_('ESHOP_VOUCHER_APPLY'); ?></button>
							</div>
						</div>
					</td>
				</tr>
	...
	?>
	<a class="button" href="<?php echo JRoute::_(EshopHelper::getContinueShopingUrl()); ?>"><?php echo JText::_('ESHOP_CONTINUE_SHOPPING'); ?></a>
	<?php
	...
	?>
	<a class="button pull-right" href="<?php echo $checkoutUrl; ?>"><?php echo JText::_('ESHOP_CHECKOUT'); ?></a>
	...
	<script type="text/javascript">
		...
		function updateCart(key)
		{
			Eshop.jQuery(function($){
				$.ajax({
					type: 'POST',
					url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.update',
					data: 'key=' + key + '&quantity=' + document.getElementById('quantity_' + key).value,
					success: function() {
						var carturl = "<?php echo JRoute::_(EshopRoute::getViewRoute('cart')); ?>";
						window.location.href = carturl.replace(/&amp;/g,'&');
				  	}
				});
			})
		}
		...
		Eshop.jQuery(function($) {
			...
			$('.eshop-remove-item-cart').bind('click', function() {
				...
				$.ajax({
					type :'POST',
					url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.remove&key=' +  id + '&redirect=1',
					success : function() {
		...
		if (EshopHelper::getConfigValue('allow_coupon'))
		{
			...
			function applyCoupon()
			{
				Eshop.jQuery(function($) {
					$.ajax({
						type: 'POST',
						url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.applyCoupon',
		...
		if (EshopHelper::getConfigValue('allow_voucher') && $this->user->get('id'))
		{
			...
			function applyVoucher()
			{
				Eshop.jQuery(function($) {
					$.ajax({
						type: 'POST',
						url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.applyVoucher',
		...
		if ($this->shipping_required)
		{
			...
			Eshop.jQuery(function($){
				$('select[name=\'country_id\']').bind('change', function() {
					$.ajax({
						url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.getZones&country_id=' + this.value,
						dataType: 'json',
						beforeSend: function() {
			...
			function applyShipping()
			{
				Eshop.jQuery(function($){
					...
					else
					{
						$.ajax({
							type: 'POST',
							url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.applyShipping',
			...
			Eshop.jQuery(function($){
				$('#get-quotes').bind('click', function() {
					$.ajax({
						type: 'POST',
						url: '<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=cart.getQuote',
